...
- [x] Settings-sidebar **Parent** `<select>` is removed (grouping is context-menu only); create-toolbar Unparent removed (Ungroup covers lift-to-root)
...
- [x] **Created** models with **no stamped mesh parts** are omitted from the zip (not packed as empty GLBs)
