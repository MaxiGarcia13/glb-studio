...
1. Add recipe modules or entries under `create/domain/kits/` (e.g. `simple-building.ts` → **Modern house** two-storey gable recipe, `block-robot.ts`, optional `simple-car`)
...
### Starter kits (content)

| Id | Label | Notes |
| ---- | ----- | ----- |
| `simple-building` | Modern house | Single body + front dormer bay; pitched roof slabs; column gable fills (pitch-matched); lit windows/door, porch, balconies — existing kinds only |
| `block-robot` | Block robot | Armature create-group tree + segment meshes |
