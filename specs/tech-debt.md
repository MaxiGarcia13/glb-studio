...
- [ ] Wire `ModelIcon` into `LibraryModelTitle` (requirements: model header shows `ModelIcon` next to the name), or delete `src/components/icons/model-icon.tsx` and amend `current/` if the icon is no longer wanted
