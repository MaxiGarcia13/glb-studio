...
- [x] **Right-click** opens **Group** / **Ungroup** / **Make joint** / **Unjoint** for create parts on a created model (library + preview), and **Group** / **Ungroup** for library models — **without changing selection** (actions use the current selection); Group creates an organizational create group; Make joint creates a skeleton joint with a suggested/custom name picker; both use the average world origin of the selection as pivot; Ungroup dissolves plain groups or lifts parts; Unjoint dissolves joints only; Skin builds bones from joints only
