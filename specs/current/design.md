...
11. **Skinned albedo (US-40, in progress):** when focus is `isSkinnedLibraryModel`, allow apply / replace / clear of `MeshStandardMaterial.map` on a resolved skinned mesh (selected `SkinnedMesh` or sole skinned mesh); shared helpers `replaceColorMap` / `clearColorMap` / `replaceMaterialColorMapFromFile` (same dispose + alpha sync as create); no US-39 prep modal. Further mesh starter kits stay content-only under `create/domain/kits/`; further skinned kits add a GLB under `public/kits/` + registry entry (US-33 pattern); soft auto-weights / weight paint deferred
