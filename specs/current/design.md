...
5. Zip basenames (US-5) follow renamed `fileName` / `name` via existing `stripExportExtension` + collision suffixes
...
6. **Filename uniqueness:** `downloadExportZip` / folder pack uniquify zip paths (`uniqueFileName` / `uniquePathSegment`) before finalize. `buildZipArchive` writes names as given and **throws** on duplicates (no silent second rename). Modal supplies Format + layout + optional basenames: zip archive (`glb-export` / `fbx-export` defaults), each group file, each ungrouped model — sanitized with `resolveZipFileName` / `resolveExportFileName(…, format)`; animation-only files keep library clip names + format extension. No **Merge visible models** checkbox / `mergeModels` opt-in — editor groups are the opt-in
