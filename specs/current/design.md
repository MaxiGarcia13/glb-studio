...
8. **Hierarchy + outliner + Group (US-26) + bones (US-31):**
   ...
   - **Imported (US-31):** `listBoneEntries` + `BoneOutliner` under each model where `isSkinnedLibraryModel` (`source === 'imported'` + usable skeleton — covers US-33 kits and post–Skin model); order **bones, then texture rows (US-46), then owned clips**; click → Edit + select bone; Shift+click toggles. Viewport `SkeletonHelper` per previewed skinned model when `$viewportSettings.bonesVisible` (`ModelSkeletonHelper` — not pickable; unmount on hide/remove or toggle off)
...
11. **Skinned albedo (US-40 / US-46):** when focus is `isSkinnedLibraryModel`, `SkinnedTextureToolbar` (bottom-center) allows apply / replace / clear of `MeshStandardMaterial.map` on a resolved skinned mesh (selected `SkinnedMesh` or sole skinned mesh via `resolveSkinnedTextureTarget` / `getSkinnedTextureAvailability`); decode via `loadSkinnedColorMapFromFile` (`flipY: false` for glTF UV atlases); assign / clear via `commitMaterialColorMapChange` (session undo); no US-39 prep modal. Under each skinned `LibraryModel`, `SkinnedTextureRows` lists textured skinned meshes (`listTexturedSkinnedMeshes`) with clear / replace; `$materialMapsRevision` keeps toolbar pressed state and Library rows in sync after apply / clear / undo / redo. Further mesh starter kits stay content-only under `create/domain/kits/`; further skinned kits add a GLB under `public/kits/` + registry entry (US-33 pattern); soft auto-weights / weight paint deferred
...
| `materialColorMap` | Color-map apply / replace / clear (US-46)  | Before/after `{ modelId, meshUuid, map clone or null + alpha flags }`; created parts + skinned albedo; live assign via `assignMaterialColorMapLive` + `releaseOrphanColorMap`; stack owns texture clones until pruned; bump `$materialMapsRevision` on commit / undo / redo; Library `SkinnedTextureRows`                                                             |
