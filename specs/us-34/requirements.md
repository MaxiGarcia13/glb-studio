...
| **Joint authoring**  | Created-model part context menu keeps **Group** / **Ungroup** and **Make connector** / **Unjoint**. Make connector is a one-step modal (mark bend points → Connect; 2+ marks form a branching limb tree; auto-names from parts). Skin builds bones only from **joints** (plus `Armature` container); plain groups stay hierarchy-only. Model library Group / Ungroup unchanged. |
...
- [ ] Created-model part multi-select (≥2) offers **Group** (organizational) and **Make connector** (mark bend points → Connect; 2+ form a branching tree; auto-names)
