...
4. Spawn parent = created model’s parts root / scene; sibling +X offset so parts are not stacked; `addPart` forces Edit + selects the mesh so TransformControls moves only that part
